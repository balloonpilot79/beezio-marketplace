create or replace function public.beezio_platform_fee_sql(p_seller_ask numeric)
returns numeric
language sql
immutable
as $$
  select round(
    least(
      greatest(
        coalesce(p_seller_ask, 0) * 0.10,
        case when coalesce(p_seller_ask, 0) < 1 then 0 else 1 end
      ),
      case when coalesce(p_seller_ask, 0) > 200 then 20 else 20 end
    )::numeric,
    2
  );
$$;

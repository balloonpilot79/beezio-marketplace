...
export const handler: Handler = async (event) => {
  try {
    ...
    if (!title || !body) return json(400, { error: 'Missing title/body' });

    const supabaseAdmin = createClient(supabaseUrl, serviceRoleKey);
    const now = new Date().toISOString();
    const { data, error } = await supabaseAdmin
      .from('admin_announcements')
      .insert({ sender_id: user.id, title, body, created_at: now } as any)
      .select('id')
      .single();
    if (error) return json(500, { error: error.message || 'Failed to create announcement' });

    return json(200, { id: String((data as any)?.id || '') });
  } catch (e) {

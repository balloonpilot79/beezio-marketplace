...
const mapProductRowToMarketplaceProduct = (row: ProductRowWithJoins): MarketplaceProduct | null => {
  ...
  const baseProduct: MarketplaceProduct = {
    ...
    reviews: reviewCount,
    commission_rate: (row as any).affiliate_payout_amount ?? row.flat_commission_amount ?? 0,
    commission_type: 'flat_rate',
    flat_commission_amount: (row as any).affiliate_payout_amount ?? row.flat_commission_amount ?? 0,
    affiliate_commission_type: 'flat',
    affiliate_commission_value: (row as any).affiliate_payout_amount ?? (row as any).affiliate_commission_value ?? 0,

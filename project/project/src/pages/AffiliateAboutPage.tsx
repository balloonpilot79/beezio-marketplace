...
interface AffiliateAboutPageProps {
  affiliateId?: string;
  isCustomDomain?: boolean;
  storeSlug?: string;
}

const AffiliateAboutPage: React.FC<AffiliateAboutPageProps> = ({ affiliateId: propAffiliateId, isCustomDomain = false, storeSlug }) => {
    ...
    const load = async () => {
      try {
        ...
        if (!storeSettingsData && lookupId) {
          const { data: settingsByAffiliate } = await supabase
            .from('affiliate_store_settings')
            .select('*')
            .eq('affiliate_id', lookupId)
            .maybeSingle();
          if (settingsByAffiliate) {
            storeSettingsData = settingsByAffiliate;
          }
        }

        const canonicalId = affiliateRecord?.id || storeSettingsData?.affiliate_id || lookupId;
        setResolvedAffiliateId(canonicalId || null);
        ...
        console.error('[AffiliateAboutPage] Error loading store:', error);
        setAffiliate(null);
  ...
  const storeName = storeSettings?.store_name || affiliate.full_name || 'Store';
  const aboutCopy = storeSettings?.store_description || affiliate.bio || 'This store is building its story.';
...
export default AffiliateAboutPage;

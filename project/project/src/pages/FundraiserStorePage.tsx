          ...
          <div className="relative z-10 flex flex-col md:flex-row items-start justify-between gap-6">
            ...
            <div className="flex items-center gap-4">
              ...
              {customPages.filter(p => p.page_slug !== 'contact').map((p) => {
                const username = String((fundraiser as any)?.username || '').trim();
                const pageUrl = username ? `/fundraiser/${username}/${p.page_slug}` : `/fundraiser/${canonicalFundraiserId || fundraiserId}`;
                return (
                  <Link
                    key={p.page_slug}
                    to={pageUrl}

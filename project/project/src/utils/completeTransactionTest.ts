...
class MarketplaceTransactionTester {
  ...
  private testProduct: TestProduct | null = null
  private testTransaction: TestTransaction | null = null
  ...
  private async createUserProfile(userId: string, userType: 'seller' | 'affiliate' | 'buyer', fullName: string): Promise<TestUser> {
    // Create profile
  ...
  private async setupStripeAccount(userId: string, userType: 'seller' | 'affiliate') {
  ...
  private async signTaxAgreements(userId: string) {
  ...
  private async affiliatePromotesItem() {
  ...
  private async buyerMakesPurchase() {
  ...
  private async verifyDashboardData() {
  ...
  private async testTaxCompliance() {

...
describe('cjBulkImportUtils', () => {
  it('calculates retail price with baked influencer reserve', () => {
    const cents = calculateRetailPriceCents(1000, { affiliate_percent: 20, affiliate_floor_cents: 200, affiliate_enabled: true, markup_type: 'flat', markup_value: 200, paypal_fee_bps: 350, paypal_fixed_cents: 65 });
    expect(cents).toBe(1938);
  });
  it('estimates shipping from weight tiers', () => {
    const tiers = [{ min_oz: 0, max_oz: 8, shipping_cents: 499 }, { min_oz: 9, max_oz: 32, shipping_cents: 699 }];
  ...
  it('derives product stock from variants', () => {
    const stock = deriveProductStock({ track_inventory: true, status: 'active', variants: [{ inventory: 0, is_active: true, in_stock: false }, { inventory: 4, is_active: true, in_stock: true }] });

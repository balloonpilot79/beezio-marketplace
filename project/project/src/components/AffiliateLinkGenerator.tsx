...
const AffiliateLinkGenerator: React.FC<AffiliateLinkGeneratorProps> = ({ affiliateId }) => {
  ...
  const generateLink = async () => {
    ...
    try {
      ...
      const shortUrl = `${baseUrl}/af/${linkCode}`;
      const fullUrl = `${baseUrl}/product/${selectedProduct}?ref=${affiliateId}&code=${encodeURIComponent(linkCode)}`;

      // Prefer modern schema: link_code + full_url
      let data: any = null;
      let error: any = null;
      ...
      // Fallback to legacy schema: link_url/custom_name
      if (error) {
        ({ data, error } = await supabase
          .from('affiliate_links')
          .insert({
      ...
      if (qrRef.current) {
        ...
        qrCodeRef.current = new QRCodeStyling({
          ...
          data:
            link.link_url ||
            link.full_url ||
            `${window.location.origin}/af/${encodeURIComponent(String(link.link_code))}`,
          margin: 10,
          qrOptions: {
            typeNumber: 0,
            mode: 'Byte',
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <div className="bg-gray-50 rounded-lg p-3 font-mono text-sm break-all mb-3">
                      {link.link_url || link.full_url || `${window.location.origin}/af/${link.link_code}`}
                    </div>

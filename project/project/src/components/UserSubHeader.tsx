...
const UserSubHeader: React.FC = () => {
  ...
  return (
    <div className="relative z-[60] bg-gradient-to-r from-purple-50 via-orange-50 to-yellow-50 border-b border-gray-100 shadow-md">
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl font-semibold text-base transition-all duration-200 shadow-sm border ${
                    isActive(item.path)
                      ? 'bg-orange-100 text-orange-700 border-orange-200 ring-2 ring-orange-300'
                      : 'text-gray-600 border-transparent hover:text-orange-700 hover:bg-orange-50 hover:border-orange-200'
                  }`}
                >
            ...
            <div className="hidden md:flex items-center gap-3">
              {quickActions[profile.role as keyof typeof quickActions]?.map((action, index) => (
                <Link
                  key={index}
                  to={action.path}
                  className={`bg-gradient-to-r from-orange-500 to-amber-500 text-white px-4 py-2 rounded-lg text-sm font-semibold shadow hover:from-orange-600 hover:to-amber-600 transition-all`}
                >
                  {action.label}
                </Link>
              ))}

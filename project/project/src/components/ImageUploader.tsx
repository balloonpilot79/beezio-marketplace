    ...
    try {
      ...
      const candidateBuckets = Array.from(
        new Set(
          [
            bucketName,
            // Common Beezio buckets (older + current)
            'store-banners',
            'profile-avatars',
            'avatars',
            'product-images',
      ...
      for (const b of candidateBuckets) {
        ...
        if (!uploadError && data?.path) {
          uploaded = { bucket: b, path: data.path };
          break;
        }
        ...
        const missingBucket = status === 404 || /bucket/i.test(msg) && (/not found/i.test(msg) || /does not exist/i.test(msg));
      ...
      if (err.message?.includes('not found') || err.message?.includes('does not exist')) {
        setError('Storage bucket not configured. Create bucket(s) in Supabase Storage or use direct URL input.');

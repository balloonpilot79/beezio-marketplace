...
const ProductGrid: React.FC<ProductGridProps> = ({ 
  ...
  const { profile, currentRole, userRoles, hasRole } = useAuth();

  const normalizedRoles = (userRoles || []).map((r) => String(r).toLowerCase());
  const activeRole = String((profile as any)?.primary_role || profile?.role || currentRole || '').toLowerCase();
  const isSellerActive = activeRole === 'seller';
  const isAffiliateActive = activeRole === 'affiliate' || activeRole === 'fundraiser';
  const canManageStore = isSellerActive || isAffiliateActive;
        ...
        {products.map((product) => {
          ...
          >
            ...
            <div className="p-4">
              ...
              {/* Store management button (role-specific) */}
              {canManageStore && (
                <div className="mt-3 mb-2">
                  {isSellerActive ? (
                    <AddToSellerStoreButton productId={normalizedProduct.id} size="sm" variant="button" />
                  ) : (

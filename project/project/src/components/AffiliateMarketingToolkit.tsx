...
export default function AffiliateMarketingToolkit() {
  ...
  const getAffiliateLink = (productId: string): string => {
    ...
    const code = (link as any).link_code || (link as any).referral_code;
    const fullUrl = (link as any).full_url || (link as any).link_url;

    if (typeof fullUrl === 'string' && fullUrl.startsWith('http')) return fullUrl;

    // Canonical: ref is the affiliate profile id; code is the share/link code.
    if (profile?.id && code) {
      return `${baseUrl}/product/${productId}?ref=${profile.id}&code=${encodeURIComponent(String(code))}`;
    }

...
// Kept as a compatibility export. Current product pricing uses fixed dollar
// tiers based on the final advertised price, never a platform percentage.
export const PLATFORM_FEE_PERCENT = DEFAULT_BEEZIO_PLATFORM_RATE * 100;
...
// Influencer bonuses are fixed per sold item:
// - advertised price under $20 => $0.50 per eligible slot
// - advertised price $20+ => $1.00 per eligible slot
...
export interface PayoutSettings {
  affiliatePercent: number; // legacy compatibility only
  platformPercent: number; // legacy compatibility only
}

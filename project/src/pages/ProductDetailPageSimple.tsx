...
const ProductDetailPageSimple: React.FC = () => {
  ...
  const imageGallery = useMemo(() => {
    ...
    if (Array.isArray(productImages)) rawImages.push(...productImages.filter(Boolean));
    if ((product as any)?.image && !rawImages.includes((product as any).image)) {
      rawImages.unshift((product as any).image);
    }

    const normalized = rawImages
      .map((image) => resolveProductImage(image, product?.id || product?.name))
      .filter(Boolean);

    return normalized.length > 0
      ? normalized.slice(0, 6)
      : [resolveProductImage(undefined, product?.id || product?.name)];
  }, [product]);
  ...
  const sellerId = (product as any)?.sellerId || (product as any)?.seller_id || (product as any)?.seller || '';
  const userRole = profile?.role;
  const canPromote = userRole === 'affiliate' || userRole === 'fundraiser';
  const canAddAsSeller = userRole === 'seller' && Boolean(user || profile);
  ...
  useEffect(() => {
    const loadProduct = async () => {
      ...
      if (sampleDataEnabled) {
        setProduct(sampleProducts.find((p) => p.id === productId) || null);
        setLoading(false);
        return;
      }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/marketplace" className="inline-flex items-center bg-orange-600 text-white px-6 py-3 rounded-lg hover:bg-orange-700 transition-colors">
  ...
  const displayImage = imageGallery[activeImageIndex] || imageGallery[0];

  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-orange-50 to-yellow-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex items-center justify-between gap-4 mb-8">
          <Link to="/marketplace" className="inline-flex items-center text-gray-600 hover:text-orange-600 font-medium transition-colors group">
          ...
          <div className="lg:col-span-6 space-y-6">
            <div className="relative bg-gradient-to-br from-yellow-100 via-orange-100 to-purple-100 rounded-3xl aspect-square flex items-center justify-center shadow-lg overflow-hidden">
              <img
                ...
                onError={(e) => {
                  e.currentTarget.src = resolveProductImage(undefined, product.id || product.name);
                }}
              />
                ...
                {imageGallery.map((img, idx) => (
                  ...
                  >
                    <img
                      ...
                      onError={(e) => {
                        e.currentTarget.src = resolveProductImage(undefined, `${product.id}-${idx}`);
                      }}
                    />
                  </button>
                ))}
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-xl p-6 md:p-8 space-y-6 sticky top-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center border rounded-lg overflow-hidden shadow-sm">
                  <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="px-3 py-2 text-gray-700 hover:bg-gray-100" aria-label="Decrease quantity">−</button>
                  <span className="px-4 py-2 text-gray-900 font-semibold min-w-[40px] text-center">{quantity}</span>
                  <button onClick={() => setQuantity(quantity + 1)} className="px-3 py-2 text-gray-700 hover:bg-gray-100" aria-label="Increase quantity">+</button>
                </div>
              </div>

              <button onClick={handleAddToCart} className="w-full inline-flex items-center justify-center bg-orange-600 text-white px-6 py-3 rounded-lg hover:bg-orange-700 transition-colors font-semibold shadow-lg">
                <ShoppingCart className="w-5 h-5 mr-2" />
                Add to Cart
              </button>

              {(canPromote || canAddAsSeller) && (
                <div className="border-t pt-5 space-y-3">
                  <p className="text-sm font-semibold text-gray-800">Add this product to your store</p>
                  {canPromote && (
                  ...
                  {canAddAsSeller && <AddToSellerStoreButton productId={product.id} size="lg" variant="button" />}
                </div>
              )}
              ...
              <div className="text-sm text-gray-500 space-y-1">
                <p>Shipping: {shippingPrice ? formatMoney(shippingPrice) : '$0.00 (seller-defined)'}</p>

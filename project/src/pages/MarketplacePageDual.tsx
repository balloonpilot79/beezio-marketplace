...
const MarketplacePageDual: React.FC = () => {
  ...
  const canAddToStore = userRole === 'seller' || userRole === 'affiliate' || userRole === 'fundraiser';
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        console.warn('MarketplacePageDual: falling back to sample data', error);
      } finally {
        if (isMounted) {
          setLoading(false);
        }
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full border transition-colors whitespace-nowrap ${
                  selectedCategory === category
                    ? 'bg-[#ffcb05] text-[#101820] border-[#ffcb05] font-semibold'
                    : 'border-gray-200 text-gray-600 hover:border-[#ffcb05] hover:text-[#101820]'
                }`}
              >

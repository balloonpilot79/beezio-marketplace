...
import { Package, Plus, Check, Search, Filter, Star, Target, Heart } from 'lucide-react';
...
const ProductBrowserForFundraisers: React.FC = () => {
  const { user } = useAuth();
  ...
  const loadSelectedProducts = async () => {
    ...
    try {
      ...
      const selected = new Set(data?.map(p => p.product_id) || []);
      const featured = new Set(data?.filter(p => p.is_featured).map(p => p.product_id) || []);
    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          ...
          display_order: selectedProducts.size
        });
      ...
      alert('Failed to add product to your fundraiser');
    } finally {
      setAdding(null);
    }
  };
  ...
  const handleRemoveProduct = async (productId: string) => {
    ...
    try {
      ...
      alert('Failed to remove product from your fundraiser');
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-green-600 to-blue-600 rounded-xl p-6 text-white">
        <h2 className="text-2xl font-bold mb-2 flex items-center gap-2">
          ...
          Browse Products for Your Fundraiser
        </h2>
        <p className="text-green-100">
          Select products to promote. 5% of each sale goes toward your fundraising goal!
        </p>
          ...
          return (
            <div
              ...
              className={`bg-white rounded-xl shadow-sm border-2 transition-all ${
                isSelected ? 'border-green-500 shadow-green-100' : 'border-gray-100 hover:border-green-200'
              }`}
            >
              {/* Product Image */}
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-green-600 font-medium">You get $0.{(product.price * 0.05).toFixed(0)} per sale</p>
                  </div>
                ...
                {!isSelected ? (
                  <button
                    ...
                    disabled={isAdding}
                    className="w-full bg-green-600 text-white py-2 rounded-lg hover:bg-green-700 transition-colors disabled:bg-gray-400 font-medium flex items-center justify-center gap-2"
                  >
                    {isAdding ? (
                      <>
...
export default ProductBrowserForFundraisers;

...
const ProductCard: React.FC<ProductCardProps> = ({ product, viewMode, affiliateRef }) => {
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm hover:shadow-xl border border-gray-100 p-6 transition-all duration-300 hover:border-purple-200 group">
        <div className="flex items-center space-x-6">
          <Link to={productUrl} className="relative block shrink-0">
            <img
              src={currentImage}
              alt={product.title}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between mb-2">
              ...
              <button onClick={handleWishlist} className="p-2 text-gray-400 hover:text-red-500 transition-colors shrink-0">
            ...
            <Link to={productUrl} className="block">
              ...
              <div className="flex items-center space-x-4 mb-3">
                ...
                <div className="flex items-center bg-green-50 px-2 py-1 rounded-full">
                  ...
                  <span className="text-xs text-green-700 font-medium">{commissionRate}% commission</span>
                </div>
              </div>
            </Link>
            ...
            <div className="flex items-center justify-between gap-4">
              <Link to={productUrl} className="min-w-0">
                ...
                <div className="text-sm text-gray-500">by {product.profiles?.full_name || 'Beezio Seller'}</div>
              </Link>

              <div className="flex flex-col items-stretch gap-2 shrink-0 relative z-10">
                <button onClick={handleAddToCart} className="bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 text-white px-6 py-3 rounded-xl font-medium transition-all duration-200 flex items-center justify-center shadow-lg hover:shadow-purple-500/25">
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm hover:shadow-2xl border border-gray-100 overflow-hidden transition-all duration-300 hover:border-purple-200 hover:-translate-y-1 group">
      <Link to={productUrl} className="block">
        <div className="relative">
          ...
          <div className="absolute top-3 left-3 flex flex-col space-y-2">
            {commissionRate > 15 && (
              <div className="bg-gradient-to-r from-orange-400 to-red-400 text-white text-xs px-3 py-1 rounded-full font-bold shadow-lg backdrop-blur-sm">Hot Deal</div>
            )}
            <div className="bg-green-500/90 backdrop-blur-sm text-white text-xs px-3 py-1 rounded-full font-medium">{commissionRate}% commission</div>
          </div>
        </div>

        <div className="p-5 pb-3">
          <div className="mb-3">
            <h3 className="text-lg font-semibold text-gray-900 group-hover:text-purple-600 transition-colors line-clamp-2 mb-2">{product.title}</h3>
            <div className="text-sm text-gray-500 mb-2">by {product.profiles?.full_name || 'Beezio Seller'}</div>
          ...
          <div className="text-xs text-gray-500">Price includes platform and commission fees.</div>
        </div>
      </Link>

      <div className="p-5 pt-2 space-y-2 relative z-10">
        <div className="flex flex-col sm:flex-row gap-2">
          <button onClick={handleAddToCart} className="flex-1 bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 text-white py-3 rounded-xl font-medium transition-all duration-200 flex items-center justify-center shadow-lg hover:shadow-purple-500/25">
            <ShoppingCart className="w-5 h-5 mr-2" />
            Add to Cart
          </button>
          <div onClick={(e) => e.stopPropagation()}>
        ...
        <Link to={productUrl} className="w-full bg-gray-50 hover:bg-gray-100 text-gray-700 py-3 rounded-xl font-medium transition-all duration-200 flex items-center justify-center border border-gray-200 hover:border-gray-300">
